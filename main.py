...
def user_only(f):
    ...
    def decorated_function(post_id):
        ...
        if not post or current_user.name != post.author.name:
            return abort(403)
        return f(post_id)        
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
    return render_template("post.html", post=requested_post,form=form,is_logged = current_user.is_authenticated)
